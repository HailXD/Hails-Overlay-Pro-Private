...
`);Oe(l).then(()=>D("Color data copied to clipboard!")).catch(()=>D("Failed to copy color data."))}),t("op-name").addEventListener("change",async n=>{const r=F();if(!r)return;const s=(n.target.value||"").trim()||"Overlay";c.overlays.some(u=>u.id!==r.id&&(u.name||"").toLowerCase()===s.toLowerCase())?(r.name=Ae(s),D(`Name in use. Renamed to "${r.name}".`)):r.name=s,await L(["overlays"]),Je()}),t("op-fetch").addEventListener("click",async()=>{const n=F();if(!n){alert("No active overlay selected.");return}if(n.imageBase64){alert("This overlay already has an image. Create a new overlay to change the image.");return}const r=t("op-image-url").value.trim();if(!r){alert("Enter an image link first.");return}try{await $t(n,r)}catch(s){console.error(s),alert("Failed to fetch image.")}});const o=t("op-dropzone");o.addEventListener("click",()=>t("op-file-input").click()),t("op-file-input").addEventListener("change",async n=>{const r=n.target.files&&n.target.files[0];if(n.target.value="",!r)return;const s=F();if(s){if(s.imageBase64){alert("This overlay already has an image. Create a new overlay to change the image.");return}try{await Qe(s,r)}catch(u){console.error(u),alert("Failed to load local image.")}}}),["dragenter","dragover"].forEach(n=>o.addEventListener(n,r=>{r.preventDefault(),r.stopPropagation(),o.classList.add("drop-highlight")})),["dragleave","drop"].forEach(n=>o.addEventListener(n,r=>{r.preventDefault(),r.stopPropagation(),!(n==="dragleave"&&r.target!==o)&&o.classList.remove("drop-highlight")})),o.addEventListener("drop",async n=>{const r=n.dataTransfer;if(!r)return;const s=r.files&&r.files[0];if(!s)return;const u=F();if(u){if(u.imageBase64){alert("This overlay already has an image. Create a new overlay to change the image.");return}try{await Qe(u,s)}catch(l){console.error(l),alert("Failed to load dropped image.")}}});const i=async(n,r)=>{const s=F();s&&(s.offsetX+=n,s.offsetY+=r,await L(["overlays"]),U(),N())};t("op-nudge-up").addEventListener("click",()=>i(0,-1)),t("op-nudge-down").addEventListener("click",()=>i(0,1)),t("op-nudge-left").addEventListener("click",()=>i(-1,0)),t("op-nudge-right").addEventListener("click",()=>i(1,0)),t("op-opacity-slider").addEventListener("input",n=>{const r=F();r&&(r.opacity=parseFloat(n.target.value),document.getElementById("op-opacity-value").textContent=Math.round(r.opacity*100)+"%")}),t("op-opacity-slider").addEventListener("change",async()=>{await L(["overlays"]),U()}),t("op-download-overlay").addEventListener("click",()=>{const n=F();if(!n||!n.imageBase64){D("No overlay image to download.");return}const r=document.createElement("a");r.href=n.imageBase64,r.download=`${(n.name||"overlay").replace(/[^\w.-]+/g,"_")}.png`,document.body.appendChild(r),r.click(),r.remove()}),t("op-open-cc").addEventListener("click",()=>{const n=F();if(!n||!n.imageBase64){D("No overlay image to edit.");return}Xt(n)}),t("op-log-coords").addEventListener("click",()=>{const n=F();if(!n){D("No active overlay selected.");return}Yt(n),D(`Logged pixel coordinates for "${n.name}" to the console.`)});const a=t("op-open-resize");a&&a.addEventListener("click",()=>{const n=F();if(!n||!n.imageBase64){D("No overlay image to resize.");return}Ut(n)}),window.addEventListener("resize",()=>{}),t("op-colors-list").addEventListener("scroll",()=>{const n=t("op-colors-list");n&&(c.colorsScrollTop=n.scrollTop,Rt())})}function _t(t){const o=t.querySelector("#op-header"),i=t.querySelector("#op-panel-toggle");if(!o||!i)return;let a=!1,n=0,r=0,s=0,u=0,l=!1;const m=(f,x,k)=>Math.min(Math.max(f,x),k),g=f=>{const x=t.classList.contains("collapsed"),k=x?i:o;if(x){if(f.target!==i)return}else if(!f.target.closest("#op-header")||f.target.closest("button"))return;a=!0,l=!1,n=f.clientX,r=f.clientY;const v=t.getBoundingClientRect();s=v.left,u=v.top,k.setPointerCapture?.(f.pointerId),f.preventDefault()},y=f=>{if(!a)return;const x=f.clientX-n,k=f.clientY-r,v=Math.max(8,window.innerWidth-t.offsetWidth-8),b=Math.max(8,window.innerHeight-t.offsetHeight-8);t.style.left=m(s+x,8,v)+"px",t.style.top=m(u+k,8,b)+"px",l=!0},w=f=>{if(!a)return;const k=t.classList.contains("collapsed")?i:o;a=!1,k.releasePointerCapture?.(f.pointerId),l&&(c.panelX=parseInt(t.style.left,10)||0,c.panelY=parseInt(t.style.top,10)||0,L(["panelX","panelY"]))};t.addEventListener("pointerdown",g),t.addEventListener("pointermove",y),t.addEventListener("pointerup",w),t.addEventListener("pointercancel",w),window.addEventListener("resize",()=>{const f=t.getBoundingClientRect(),x=Math.max(8,window.innerWidth-t.offsetWidth-8),k=Math.max(8,window.innerHeight-t.offsetHeight-8),v=Math.min(Math.max(f.left,8),x),b=Math.min(Math.max(f.top,8),k);t.style.left=v+"px",t.style.top=b+"px",c.panelX=v,c.panelY=b,L(["panelX","panelY"])}),i.addEventListener("click",f=>{if(l){f.preventDefault(),f.stopPropagation();return}c.isPanelCollapsed=!c.isPanelCollapsed,L(["isPanelCollapsed"]),N()})}function _e(){document.body.classList.toggle("op-theme-dark",c.theme==="dark"),document.body.classList.toggle("op-theme-light",c.theme!=="dark");const t=document.getElementById("op-toast-stack");t&&t.classList.toggle("op-dark",c.theme==="dark")}function Pt(){const t=y=>document.getElementById(y),o=F(),i=t("op-editor-section"),a=t("op-editor-body");if(i.style.display=o?"flex":"none",!o)return;t("op-name").value=o.name||"";const n=t("op-image-source"),r=t("op-preview-wrap"),s=t("op-image-preview"),u=t("op-cc-btn-row");o.imageBase64?(n.style.display="none",r.style.display="flex",s.src=o.imageBase64,u.style.display="flex"):(n.style.display="block",r.style.display="none",u.style.display="none",t("op-image-url").value=o.imageUrl||"");const l=o.pixelUrl?de(o.pixelUrl):{chunk1:"-",chunk2:"-",posX:"-",posY:"-"};t("op-coord-display").textContent=o.pixelUrl?`Ref: chunk ${l.chunk1}/${l.chunk2} at (${l.posX}, ${l.posY})`:'No pixel anchor set. Turn ON "Place overlay" and click a pixel once.',t("op-opacity-slider").value=String(o.opacity),t("op-opacity-value").textContent=Math.round(o.opacity*100)+"%";const m=document.getElementById("op-offset-indicator");m&&(m.textContent=`Offset X ${o.offsetX}, Y ${o.offsetY}`),a.style.display=c.collapseEditor?"none":"block";const g=document.getElementById("op-collapse-editor");g&&(g.textContent=c.collapseEditor?"\u25B8":"\u25BE")}async function N(){const t=_=>document.getElementById(_),o=t("overlay-pro-panel");if(!o)return;_e();const i=t("op-content"),a=t("op-panel-toggle"),n=t("op-header"),r=!!c.isPanelCollapsed;o.classList.toggle("collapsed",r),i.style.display=r?"none":"grid",a.classList.toggle("logo-button",r),a.title=r?"Expand":"Collapse";const s=t("op-mode-select");s&&(s.value=c.overlayMode);const u=t("op-autocap-toggle"),l=t("op-place-label");u.textContent=c.autoCapturePixelUrl?"ON":"OFF",u.classList.toggle("op-danger",!!c.autoCapturePixelUrl),l.classList.toggle("op-danger-text",!!c.autoCapturePixelUrl);const m=t("op-highlight-toggle");m&&(m.checked=!!c.highlightPixels);const g=t("op-hijack-toggle");g&&(g.checked=!!c.hijackRequests);const y=t("op-pixel-count");y&&(y.value=c.hijackPixelCount||1);const w=g?.nextElementSibling;w&&w.classList.toggle("op-danger-text",!!c.hijackRequests);const f=t("op-list-wrap"),x=t("op-collapse-list");f.style.display=c.collapseList?"none":"block",x&&(x.textContent=c.collapseList?"\u25B8":"\u25BE");const k=t("op-nudge-body"),v=t("op-collapse-nudge");k.style.display=c.collapseNudge?"none":"block",v&&(v.textContent=c.collapseNudge?"\u25B8":"\u25BE");const b=t("op-colors-body"),E=t("op-collapse-colors");b&&(b.style.display=c.collapseColors?"none":"block"),E&&(E.textContent=c.collapseColors?"\u25B8":"\u25BE");const I=t("op-color-sort-select");I&&(I.value=c.colorSortBy||"errorCount"),Je(),Pt();const S=t("op-export-overlay"),C=F(),$=!!(C&&C.imageUrl&&!C.isLocal);S.disabled=!$,S.title=$?"Export active overlay JSON":"Export disabled for local images"}const Pe=new Map;let ie=[];async function Te(t){if(!t||!t.imageBase64)return{};const o=t.imageBase64.slice(0,64)+":"+t.imageBase64.length;if(Pe.has(o))return Pe.get(o);const i=await le(t.imageBase64),n=V(i.width,i.height).getContext("2d",{willReadFrequently:!0});n.drawImage(i,0,0);const s=n.getImageData(0,0,i.width,i.height).data,u={};for(let l=0;l<s.length;l+=4)if(s[l+3]>0){const m=`${s[l]},${s[l+1]},${s[l+2]}`;u[m]=(u[m]||0)+1}return Pe.set(o,u),u}async function Tt(t){if(!t||!t.imageBase64||!t.pixelUrl)return null;const o=Re(t),i=Array.from(pe.keys()).sort().join(";"),a=`${t.id}|${o}|${i}`;if(ve.has(a))return ve.get(a);const n=await le(t.imageBase64),r=n.width,s=n.height,l=V(r,s).getContext("2d",{willReadFrequently:!0});l.drawImage(n,0,0);const g=l.getImageData(0,0,r,s).data,y=de(t.pixelUrl);if(!Number.isFinite(y.chunk1)||!Number.isFinite(y.chunk2))return null;const w={};for(let f=0;f<s;f++)for(let x=0;x<r;x++){const k=(f*r+x)*4;if(g[k+3]===0)continue;const v=g[k],b=g[k+1],E=g[k+2],I=`${v},${b},${E}`;w[I]||(w[I]={smart:0,below:0});const S=y.chunk1*1e3+y.posX+t.offsetX+x,C=y.chunk2*1e3+y.posY+t.offsetY+f,$=Math.floor(S/1e3),_=Math.floor(C/1e3),z=`${$},${_}`;if(pe.has(z)){const R=pe.get(z),P=R.data,Y=R.width,p=(S%1e3+1e3)%1e3,M=((C%1e3+1e3)%1e3*Y+p)*4,A=P[M],O=P[M+1],X=P[M+2];P[M+3]===0?w[I].below++:(v!==A||b!==O||E!==X)&&w[I].smart++}}return ve.set(a,w),w}async function Yt(t){if(!t||!t.imageBase64||!t.pixelUrl){D("Overlay is missing image or pixelUrl to log coordinates.");return}const o=await le(t.imageBase64),i=o.width,a=o.height,n=de(t.pixelUrl);if(!Number.isFinite(n.chunk1)||!Number.isFinite(n.chunk2)){D("Invalid pixelUrl in overlay.");return}const r=(s,u,l)=>{if(s<0||s>=i||u<0||u>=a){console.log(`${l} Overlay Pixel: (${s}, ${u}) is out of bounds.`);return}const m=n.chunk1*1e3+n.posX+t.offsetX+s,g=n.chunk2*1e3+n.posY+t.offsetY+u,y=Math.floor(m/1e3),w=Math.floor(g/1e3),f=(m%1e3+1e3)%1e3,x=(g%1e3+1e3)%1e3;console.log(`${l} Overlay Pixel: (${s}, ${u}) -> Tile: (${y}, ${w}), Coordinate in Tile: (${f}, ${x})`)};console.log(`--- Specific Pixel Coordinates for Overlay: ${t.name} ---`),r(0,0,"Top-Left Corner:"),r(2,0,"Top-Left (+2 right):"),r(0,2,"Top-Left (+2 down):"),r(i-1,0,"Top-Right Corner:"),r(i-3,0,"Top-Right (-2 left):"),r(i-1,2,"Top-Right (+2 down):"),r(0,a-1,"Bottom-Left Corner:"),r(2,a-1,"Bottom-Left (+2 right):"),r(0,a-3,"Bottom-Left (-2 up):"),r(i-1,a-1,"Bottom-Right Corner:"),r(i-3,a-1,"Bottom-Right (-2 left):"),r(i-1,a-3,"Bottom-Right (-2 up):"),console.log("--- End of Specific Pixel Coordinates ---")}async function te(t=!0){const o=Ue(),i=F(),a=document.getElementById("op-colors-section");if(!a)return;if(!i||!i.imageBase64){a.style.display="none",ie=[];return}a.style.display="flex";const n=document.getElementById("op-colors-list");let r;if(t){n.innerHTML='<div class="op-muted" style="text-align:center; padding: 12px 0;">Loading...</div>';const v=await Te(i),b=await Tt(i)||{};r=Object.entries(v).map(([E,I])=>{const S=b[E]?.below||0,C=b[E]?.smart||0,$=S+C,_=I-$;return{key:E,name:me[E]||E,totalCount:I,belowCount:S,smartCount:C,errorCount:$,correctCount:_}}),Ce.set(i.id,r),ie=r}else{if(Ce.has(i.id))r=Ce.get(i.id);else{const v=await Te(i);r=Object.entries(v).map(([b,E])=>({key:b,name:me[b]||b,totalCount:E,belowCount:0,smartCount:0,errorCount:0,correctCount:E}))}ie=r}const s=c.colorSortBy||"errorCount";r=[...r].sort((v,b)=>b[s]===v[s]?b.totalCount-v.totalCount:b[s]-v[s]);let l;i.visibleColorKeys===null||i.visibleColorKeys===void 0?l=new Set(r.map(v=>v.key)):l=new Set(i.visibleColorKeys);const m=r.reduce((v,b)=>v+b.correctCount,0),g=r.reduce((v,b)=>v+b.errorCount,0),y=r.reduce((v,b)=>v+b.totalCount,0),w=document.getElementById("op-total-correct"),f=document.getElementById("op-total-wrong"),x=document.getElementById("op-total-pixels");w&&(w.textContent=m.toLocaleString()),f&&(f.textContent=g.toLocaleString()),x&&(x.textContent=y.toLocaleString());const k=new Set(oe.map(([v,b,E])=>`${v},${b},${E}`));if(n.innerHTML="",r.length===0){n.innerHTML='<div class="op-muted" style="text-align:center; padding: 12px 0;">No colors found.</div>';return}for(const v of r){const{key:b,name:E,totalCount:I,belowCount:S,smartCount:C,errorCount:$,correctCount:_}=v,z=k.has(b),R=document.createElement("div");R.className="op-dist-item"+(z?" premium":""),R.title=`${E} (${b}): ${I} pixels`;const P=`<span style="color: lime;">${_}</span>/<span style="color: cyan;">${S}</span>/<span style="color: pink;">${C}</span>/<span style="color: red;">${$}</span>/${I}`;R.innerHTML=`
        ...
        `;const Y=R.querySelector(".op-color-list-name");Y&&o.has(b)&&(Y.style.color="red"),n.appendChild(R)}n.querySelectorAll('input[type="checkbox"]').forEach(v=>{v.addEventListener("change",async()=>{const b=v.dataset.key,E=F();if(!E)return;if(E.visibleColorKeys===null){const S=Object.keys(await Te(E));E.visibleColorKeys=S}const I=new Set(E.visibleColorKeys);v.checked?I.add(b):I.delete(b),E.visibleColorKeys=Array.from(I),await L(["overlays"]),U()})}),Number.isFinite(c.colorsScrollTop)&&(n.scrollTop=c.colorsScrollTop)}let d=null;function Bt(){const t=document.createElement("div");t.className="op-cc-backdrop",t.id="op-cc-backdrop",document.body.appendChild(t);const o=document.createElement("div");o.className="op-cc-modal",o.id="op-cc-modal",o.style.display="none",o.innerHTML=`
    ...
    `,document.body.appendChild(o),o.querySelector("#op-cc-close").addEventListener("click",Ie),t.addEventListener("click",Ie),o.querySelector("#op-cc-cancel").addEventListener("click",Ie),d={backdrop:t,modal:o,previewCanvas:o.querySelector("#op-cc-preview"),previewCtx:o.querySelector("#op-cc-preview").getContext("2d",{willReadFrequently:!0}),sourceCanvas:null,sourceCtx:null,sourceImageData:null,processedCanvas:null,processedCtx:null,freeGrid:o.querySelector("#op-cc-free-grid"),paidGrid:o.querySelector("#op-cc-paid-grid"),freeToggle:o.querySelector("#op-cc-free-toggle"),paidToggle:o.querySelector("#op-cc-paid-toggle"),meta:o.querySelector("#op-cc-meta"),applyBtn:o.querySelector("#op-cc-apply"),recalcBtn:o.querySelector("#op-cc-recalc"),realtimeBtn:o.querySelector("#op-cc-realtime"),zoom:1,selectedFree:new Set(c.ccFreeKeys),selectedPaid:new Set(c.ccPaidKeys),realtime:!!c.ccRealtime,overlay:null,lastColorCounts:{},isStale:!1},d.realtimeBtn.addEventListener("click",async()=>{d.realtime=!d.realtime,d.realtimeBtn.textContent=`Realtime: ${d.realtime?"ON":"OFF"}`,d.realtimeBtn.classList.toggle("op-danger",d.realtime),c.ccRealtime=d.realtime,await L(["ccRealtime"]),d.realtime&&d.isStale&&r()});const i=async()=>{d.zoom=Math.min(8,(d.zoom||1)*1.25),c.ccZoom=d.zoom,await L(["ccZoom"]),re(),se()},a=async()=>{d.zoom=Math.max(.1,(d.zoom||1)/1.25),c.ccZoom=d.zoom,await L(["ccZoom"]),re(),se()};o.querySelector("#op-cc-zoom-in").addEventListener("click",i),o.querySelector("#op-cc-zoom-out").addEventListener("click",a),d.recalcBtn.addEventListener("click",()=>{r()}),d.applyBtn.addEventListener("click",async()=>{const s=d.overlay;if(!s)return;if(tt().length===0){D("Select at least one color.");return}if(d.isStale&&r(),!d.processedCanvas){D("Nothing to apply.");return}if(d.processedCanvas.width>=1e3||d.processedCanvas.height>=1e3){D("Image too large to apply (must be < 1000\xD71000).");return}const l=d.processedCanvas.toDataURL("image/png");s.imageBase64=l,s.imageUrl=null,s.isLocal=!0,await L(["overlays"]),U(),j(),N();const m=Object.keys(d.lastColorCounts).length;D(`Overlay updated (${d.processedCanvas.width}\xD7${d.processedCanvas.height}, ${m} colors).`),Ie()}),Nt(),d.freeToggle.addEventListener("click",async()=>{const s=at();it("free",!s),c.ccFreeKeys=Array.from(d.selectedFree),await L(["ccFreeKeys"]),d.realtime?r():n(),re(),se(),ye()}),d.paidToggle.addEventListener("click",async()=>{const s=nt();it("paid",!s),c.ccPaidKeys=Array.from(d.selectedPaid),await L(["ccPaidKeys"]),d.realtime?r():n(),re(),se(),ye()});function n(){d.isStale=!0,d.meta.textContent=d.meta.textContent.replace(/ \| Status: .+$/,"")+" | Status: pending recalculation"}function r(){Se(),d.isStale=!1,re(),se(),ot()}}function Xt(t){if(!d)return;d.overlay=t,document.body.classList.add("op-scroll-lock"),d.zoom=Number(c.ccZoom)||1,d.realtime=!!c.ccRealtime,d.realtimeBtn.textContent=`Realtime: ${d.realtime?"ON":"OFF"}`,d.realtimeBtn.classList.toggle("op-danger",d.realtime);const o=new Image;o.onload=()=>{d.sourceCanvas||(d.sourceCanvas=document.createElement("canvas"),d.sourceCtx=d.sourceCanvas.getContext("2d",{willReadFrequently:!0})),d.sourceCanvas.width=o.width,d.sourceCanvas.height=o.height,d.sourceCtx.clearRect(0,0,o.width,o.height),d.sourceCtx.drawImage(o,0,0),d.sourceImageData=d.sourceCtx.getImageData(0,0,o.width,o.height),d.processedCanvas||(d.processedCanvas=document.createElement("canvas"),d.processedCtx=d.processedCanvas.getContext("2d")),Se(),d.isStale=!1,re(),se(),ot(),d.backdrop.classList.add("show"),d.modal.style.display="flex"},o.src=t.imageBase64}function Ie(){d&&(d.backdrop.classList.remove("show"),d.modal.style.display="none",d.overlay=null,document.body.classList.remove("op-scroll-lock"))}function Ft(t,o,i,a){let n=null,r=1/0;for(let s=0;s<a.length;s++){const[u,l,m]=a[s],g=(u+t)/2,y=u-t,w=l-o,f=m-i,x=(512+g)*y*y>>8,k=4*w*w,v=(767-g)*f*f>>8,b=x+k+v;b<r&&(r=b,n=[u,l,m])}return n||[0,0,0]}function tt(){const t=[];return d.selectedFree.forEach(o=>{const[i,a,n]=o.split(",").map(r=>parseInt(r,10));Number.isFinite(i)&&t.push([i,a,n])}),d.selectedPaid.forEach(o=>{const[i,a,n]=o.split(",").map(r=>parseInt(r,10));Number.isFinite(i)&&t.push([i,a,n])}),t}function Se(){if(!d.sourceImageData)return;const t=d.sourceImageData.width,o=d.sourceImageData.height,i=d.sourceImageData.data,a=new Uint8ClampedArray(i.length),n=tt(),r={};for(let u=0;u<i.length;u+=4){const l=i[u],m=i[u+1],g=i[u+2];if(i[u+3]===0){a[u]=0,a[u+1]=0,a[u+2]=0,a[u+3]=0;continue}const[w,f,x]=n.length?Ft(l,m,g,n):[l,m,g];a[u]=w,a[u+1]=f,a[u+2]=x,a[u+3]=255;const k=`${w},${f},${x}`;r[k]=(r[k]||0)+1}d.processedCanvas||(d.processedCanvas=document.createElement("canvas"),d.processedCtx=d.processedCanvas.getContext("2d")),d.processedCanvas.width=t,d.processedCanvas.height=o;const s=new ImageData(a,t,o);d.processedCtx.putImageData(s,0,0),d.lastColorCounts=r}function re(){const t=Number(d.zoom)||1,o=d.processedCanvas;if(!o)return;const i=Math.max(1,Math.round(o.width*t)),a=Math.max(1,Math.round(o.height*t));d.previewCanvas.width=i,d.previewCanvas.height=a;const n=d.previewCtx;n.clearRect(0,0,i,a),n.imageSmoothingEnabled=!1,n.drawImage(o,0,0,o.width,o.height,0,0,i,a),n.imageSmoothingEnabled=!0}function se(){if(!d.sourceImageData){d.meta.textContent="";return}const t=d.sourceImageData.width,o=d.sourceImageData.height,i=Object.keys(d.lastColorCounts||{}).length,a=d.isStale?"pending recalculation":"up to date";d.meta.textContent=`Size: ${t}\xD7${o} | Zoom: ${d.zoom.toFixed(2)}\xD7 | Colors: ${i} | Status: ${a}`}function ot(){if(!d||!d.lastColorCounts)return;const t=document.getElementById("op-cc-color-list");if(!t)return;const o=Ue(),i=d.lastColorCounts,a=Object.entries(i).sort(([,r],[,s])=>s-r),n=new Set(oe.map(([r,s,u])=>`${r},${s},${u}`));if(t.innerHTML="",a.length===0){t.innerHTML='<div class="op-muted" style="text-align:center; padding: 12px 0;">No colors in image.</div>';return}for(const[r,s]of a){const u=n.has(r),l=me[r]||r,m=document.createElement("div");m.className="op-color-list-item"+(u?" premium":""),m.title=`${l} (${r}): ${s} pixels`,m.innerHTML=`
      ...
      `;const g=m.querySelector(".op-color-list-name");g&&o.has(r)&&(g.style.color="red"),t.appendChild(m)}}function Nt(){d.freeGrid.innerHTML="",d.paidGrid.innerHTML="";for(const[t,o,i]of xe){const a=`${t},${o},${i}`,n=document.createElement("div");n.className="op-cc-cell",n.style.background=`rgb(${t},${o},${i})`,n.title=me[a]||a,n.dataset.key=a,n.dataset.type="free",d.selectedFree.has(a)&&n.classList.add("active"),n.addEventListener("click",async()=>{d.selectedFree.has(a)?d.selectedFree.delete(a):d.selectedFree.add(a),n.classList.toggle("active",d.selectedFree.has(a)),c.ccFreeKeys=Array.from(d.selectedFree),await L(["ccFreeKeys"]),d.realtime?Se():d.isStale=!0,re(),se(),ye()}),d.freeGrid.appendChild(n)}for(const[t,o,i]of oe){const a=`${t},${o},${i}`,n=document.createElement("div");n.className="op-cc-cell",n.style.background=`rgb(${t},${o},${i})`,n.title=me[a]||a,n.dataset.key=a,n.dataset.type="paid",d.selectedPaid.has(a)&&n.classList.add("active"),n.addEventListener("click",async()=>{d.selectedPaid.has(a)?d.selectedPaid.delete(a):d.selectedPaid.add(a),n.classList.toggle("active",d.selectedPaid.has(a)),c.ccPaidKeys=Array.from(d.selectedPaid),await L(["ccPaidKeys"]),d.realtime?Se():d.isStale=!0,re(),se(),ye()}),d.paidGrid.appendChild(n)}ye()}function ye(){d.freeToggle.textContent=at()?"Unselect All":"Select All",d.paidToggle.textContent=nt()?"Unselect All":"Select All"}function at(){return we.every(t=>d.selectedFree.has(t))}function nt(){const t=oe.map(([o,i,a])=>`${o},${i},${a}`);return t.every(o=>d.selectedPaid.has(o))&&t.length>0}function it(t,o){if(t==="free")o?we.forEach(a=>d.selectedFree.add(a)):d.selectedFree.clear(),d.freeGrid.querySelectorAll(".op-cc-cell").forEach(a=>a.classList.toggle("active",o));else{const i=oe.map(([a,n,r])=>`${a},${n},${r}`);o?i.forEach(a=>d.selectedPaid.add(a)):d.selectedPaid.clear(),d.paidGrid.querySelectorAll(".op-cc-cell").forEach(a=>a.classList.toggle("active",o))}}let e=null;function qt(){const t=document.createElement("div");t.className="op-rs-backdrop",t.id="op-rs-backdrop",document.body.appendChild(t);const o=document.createElement("div");o.className="op-rs-modal",o.id="op-rs-modal",o.style.display="none",o.innerHTML=`
